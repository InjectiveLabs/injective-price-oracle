...
func newScheduler(p *Pipeline, run *Run, vars Vars, lggr log.Logger) *scheduler {
	lggr = lggr.WithField("svc", "Scheduler")
	...
	for id, task := range p.Tasks {
		...
		lggr.Debugln("scheduling task run", "dot_id", task.DotID(), "attempts", run.attempts)
		...
		if result.Attempts < uint(result.Task.TaskRetries()) && result.Result.Error != nil {
			...
			go func() {
				...
				case <-time.After(backoff.ForAttempt(float64(result.Attempts - 1))): // we subtract 1 because backoff 0-indexes
					...
					s.logger.Debugln("scheduling task run", "dot_id", run.task.DotID(), "attempts", run.attempts)
					s.taskCh <- run
				}
			}()
...
func (s *scheduler) report(ctx context.Context, result TaskRunResult) {
	...
	case <-ctx.Done():
		s.logger.Errorln("pipeline.scheduler: discarding result; report context timed out", "result", result, "err", ctx.Err())
	}
}
